...
| 5.1.0 | link:https://github.com/lucidworks/connectors-sdk-resources/releases/tag/v2.0.0[2.0.0]
...
Important point to be considered on server side when building a connector using a sdk:
* Connector server does not guarantee the order of processing of items such as Candidate, Document, Delete, etc, which are emitted from the connector plugin, therefore,
any logic that assumes the item is processed and proceeds to process the same item immediately based upon previous processing results may produce incorrect results.
For example, when a document replace is immediately followed by a delete by query, and, delete by query depends upon the result of document replace which was just before issued
to the connector server, may produce incorrect results, if delete by query is processed before a document replace by the connector server.
