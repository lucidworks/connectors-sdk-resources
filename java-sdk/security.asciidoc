...
See the main link:./java-sdk/security.html[SSL README] for general instructions on how to configure SSL/TLS support for Fusion.

== Java Plugin Client
The information here is specific to running a Java SDK plugin outside of Fusion.

The link:plugin-client.md[plugin-client] supports several variations of SSL/TLS auth. The examples below show the relevant Java properties.
